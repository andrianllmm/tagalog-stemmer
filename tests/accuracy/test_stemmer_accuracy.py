...
def test_stemmer_accuracy():
        ...
        for example in examples:
            ...
            if example["stem"] == attempt:
                ...
                if len(attempt) > len(example["stem"]):
                    results["understemming_total"] += len(attempt) - len(
                        example["stem"]
                    )
                else:
    ...
    if total_attempts > 0:
        results["understemming_avg"] = round(
            results["understemming_total"] / total_attempts, 2
        )
        results["overstemming_avg"] = round(
            results["overstemming_total"] / total_attempts, 2
        )
    ...
    if correct:
        with open(correct_file_path, "w", newline="") as correct_file:
            ...
            writer = csv.DictWriter(
                correct_file, fieldnames=fieldnames, extrasaction="ignore"
            )
            writer.writeheader()
    ...
    if incorrect:
        with open(incorrect_file_path, "w", newline="") as incorrect_file:
            ...
            writer = csv.DictWriter(
                incorrect_file, fieldnames=fieldnames, extrasaction="ignore"
            )
            writer.writeheader()
